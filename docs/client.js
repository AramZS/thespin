...
function setUpAPIRequest(aMethod, url) {
	...
	console.log("request set up", theUrl);
	theUrl = "/json/" + url + ".json";
	theMethod = aMethod;
...
const selectCharacter = function (charId, user) {
	var xhr = setUpAPIRequest(
		"POST",
		"character/" + encodeURIComponent(charId)
	);
	/**
  var xhr = new XMLHttpRequest(),
      method = "POST",
...
var getCharacters = function () {
  ...
  **/
		...
		console.log(e);
		window.alert(
			"We can not reach the server. Are you sure you are on HTTPS?"
		);
	}
};
